...
标准模式下的应用栏，页面内容向下滚动应用栏消失，内容向上滚动应用栏出现。
...
突出模式下的应用栏，页面内容向下滚动应用栏消失，内容向上滚动应用栏出现。
...
注意事项：突出模式的顶部应用栏在容纳图像时不应使用容易使应用栏图标和文字变得难以辨认的图片。

推荐：
![image](../../static/img/appBar1.png)
不推荐:
![image](../../static/img/appBar2.png)

### 密集模式(dense)

需要以更小的高度展示应用栏时，可以使用密集。

密集模式下的应用栏，页面内容向下滚动应用栏消失，内容向上滚动应用栏出现。
...
### 固定突出模式(fixed + prominent)

固定突出模式下的应用栏，页面内容发生滚动时变成标准应用栏，内容滚动到顶部时变成突出应用栏。
...
以下示例滚动目标元素为 window,当浏览器窗口滚动时，应用栏会应用对应滚动效果。
