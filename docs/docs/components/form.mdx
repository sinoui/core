...
Material Design 推荐采用浮动标签，如下示例，表单中采用浮动标签的轮廓输入框：
...
TextInput 本身集成了标签，可以作为表单控件使用。当然，也可以使用`FormControl`来组织标签与输入框之前的关系：
...
大部分输入框、选择框组件会自动浮动标签功能，简化在表单中的使用。
