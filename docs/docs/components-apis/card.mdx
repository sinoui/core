...
Card 组件可以作为卡片的主操作区，不内置支持各种交互形态（悬停、聚焦和按下），需要开发者自己实现。

### 属性

| 属性名    | 默认值   | 类型                | 描述                                                              |
| --------- | -------- | ------------------- | ----------------------------------------------------------------- |
| outlined  | false    | boolean             | 设置为`true`，表示是边框模式的卡片。                              |
| elevation | 1 或者 0 | number              | 设置海拔（即阴影程度） 。默认为 1，但如果是边框模式，则默认为 0。 |
| className |          | string              | 自定义样式类                                                      |
| style     |          | React.CSSProperties | 内联样式                                                          |
| onClick   |          |                     | 点击事件                                                          |
| as        | 'div'    | React.ElementType     | 指定根元素                                                        |
...
组件是整个卡片的主操作区，支持各种交互形态（悬停、聚焦和按下）。
...
CardActions 组件中只有 CardActionButtons 时可简化为:
...
| 属性名    | 默认值 | 类型                | 描述                       |
| --------- | ------ | ------------------- | -------------------------- |
| imageUrl  |        | string              | 必须指定。设置图片的链接。 |
| square    | false  | boolean             | 显示为 1:1 的正方形。      |
| wide      | false  | boolean             | 显示为 16:9 的全宽模式。   |
| width     |        | number              | 指定宽度。单位为 px。      |
| style     |        | React.CSSProperties | 内联样式                   |
| className |        | string              | 自定义样式类               |
| as        | 'div'  | React.ElementType     | 指定根组件                 |
...
| 属性名    | 默认值 | 类型            | 描述                   |
| --------- | ------ | --------------- | ---------------------- |
| avatar    |        | React.ReactNode | 头部图像。             |
| action    |        | React.ReactNode | 卡片头部中显示的操作。 |
| title     |        | string          | 卡片头部的主标题       |
| subheader |        | string          | 卡片头部的副标题       |
...
更多示例请查看[Card 开发文档](../components/card)
