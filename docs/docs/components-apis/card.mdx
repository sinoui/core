...
CardPrimaryAction组件是整个卡片的主操作区，支持各种交互形态（悬停、聚焦和按下）。
...
说明：和Card组件展示的示例区别为该示例交互时有动态效果。

### CardActions 

引入方式：`import CardPrimaryAction from '@sinoui/core/CardPrimaryAction ';`

CardActions组件是整个卡片的辅助操作区域。
...
CardMedia组件是用来呈现多媒体内容的组件，目前只支持显示图片。如果需要支持显示视频，需要由开发人员自行实现。
