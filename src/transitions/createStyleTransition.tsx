...
export default function createStyleTransition<Props extends {}>(
  ...
  return class StyleTransition
    ...
    get easing():
      ...
      return getEasing(easing, options.easing);
    }

    public getTimeout(node: HTMLElement): any {
      const { timeout = options.duration } = this.props;
      if (timeout === 'auto') {
      ...
      return (
        <CSSTransition
          ...
          timeout={
            (timeout === 'auto'
              ? 250
              : getDuration(timeout, options.duration)) as any
          }
